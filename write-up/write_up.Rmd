...
For the predictive model, we used a dataset that summarized contestant information of total tweet count and average tweet sentiment. We only looked at 3 seasons of data, which included the Bachelor (2018-2019) and the Bachelorette (2019).
...
A better model would have included data from all seasons of the bachelor and bachelorette. Although we created the model, ethically, we must address the issue of making a model with so few data points and drawing conclusions from it. We are therefore unable to draw any meaningful conclusions from the model but are framing this model as an approach to how the model could look with all of the seasons of data (50+ seasons). 
...
The results are highly questionable, due to the extremely high accuracy. We therefore are extremely doubutful of the results but instead view the data as a positive signal for the potential of adding much more data to the model.
```{r, out.width = "100%"}
knitr::include_graphics("chart.png")
```

## Conclusions
We observe that although no long-term predictions can be drawn from 4 seasons of bachelor franchise data, we are still able to set-up the foundations for a potential predictive process and look at long-term modeling possibilities (when using more data). We are also able to draw interesting conclusions through the use of interactive visuals. From the visuals, we gain a clear understanding on the geographic distribution of the tweets and how polarity varies across states. We were also able to see the most used words when discussing different seasons of the bachelor/bachelorette. From all of this information, we can judge how people on twitter are feeling from season to season of the show. Especially meaningful from our data is the conclusions drawn from polarity averages in each state.


## Next Steps
More season's worth of data would lead to even more insightful visualizations and a reliable predictive model. This would require Twitter data from 2002 - 2017 to be available, which we could access with a paid developer account through twitter. Furthermore, hyperparameters in each classification model can be tuned to yield better results. Additionally, looking at polarity in specific major cities could lead to insights. In our current project, we looked at tweet volume by state but it may be more meaningful in the future to look at tweet volume as proportional to population of the state. Data from more seasons would greatly increase the number and quality of conclusions we are able to draw from the bachelor franchise data.

## New Insights
In order to complete this project, we had to learn how to interpret JSON's and parse them into CSV's. Accessing data through a supercomputer (ssh-ing through the machine) was a major hurdle in intially collecting data and a new process that the team member's learned a lot from. Completing this project required gaining a better understanding of the 5 classification models mentioned previously mentioned. String manipulation was a new technique implemented when data wrangling. Furthermore, generating the visualizations required learning Tableau, which none of the team members had experience in. 
