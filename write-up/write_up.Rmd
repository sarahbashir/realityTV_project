...
all_Data<-do.call("rbind", list(bachelor_2018, bachelor_2019, bachelorette_2018, 
                                bachelorette_2019))
...
data_bachelor_2018 <- data.frame(category = sample(sentiment_bachelor_2018, 100, 
...
bad_words <- c("the", "https", "t.co", "to", "a", "you", "and", "for", "of", "on", "that", 
               "so", "my", "just", "be", "her", "me", "are", "she", "him", "his", "was", 
               "all", "have", "not", "but", "what", "how", "get", "out", "at", "your",
               "can", "we", "why", "when", "do", "go", "has", "as", "from", "know", 
               "don't", "who", "an", "or", "been", "than", "does", "she's", "you're", 
               "because", "every", "by", "else", "also", "could", "says", "he's", 
               "doesn't", "2", "that's", "thought", "any", "some", "us", "those", "1", 
               "3", "he", "even",  "let", "very", "gt", "ep", "4", "own", "45", 
               "5hglkw0p6v", "5xjk4v4zlt", "6zrkelnyzn", "9kpefew9y0", "9pcn9tykkp", 
               "dx8kzlwcyo", "our", "i'm", "isn't", "if", "it", "it's", "is", "with", 
               "in", "frcdc", "i've", "i'll", "such", "i'd", "did")
...
Access to more seasons or more data would allow us to create even more insightful visualizations. This would require Twitter data from 2006 - 2017 (Twitter founded in 2006), to be available, which we could access with a paid developer account through Twitter [@history.com_2019]. Additionally, looking at polarity in specific major cities could lead to more specific insights between more specific locations. In our current project, we looked at tweet volume by state; however, it may be more meaningful in the future to look at tweet volume as proportional to population of the state. Additionally, for the word count data set, in this case, it was fine to manually filter out the words we didn't want to use in Tableau and R since we didn't have that many stop words. In the future, especially when looking at more data, it would be usefful to use an R package that filters out stop words. Data from more seasons would greatly increase the number and quality of conclusions we are able to draw from the Bachelor franchise data.
